...
READ_AGAIN:
    ...
    /*debug*/
    MY_DEBUG("%s,%d: uart recv data is \n",__func__,__LINE__);
    for(int i=0; i<12; i++)
...
int write_to_file(char *buff)
{
  ...
  /*short address*/
    ...
    /*for debug*/
    MY_DEBUG("%s,%d: write msg:\n",__func__,__LINE__);
    MY_DEBUG("MAC [ %02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X] \n",
             ym_zigbee_dev.mac[0],ym_zigbee_dev.mac[1],ym_zigbee_dev.mac[2],ym_zigbee_dev.mac[3],
             ym_zigbee_dev.mac[4],ym_zigbee_dev.mac[5],ym_zigbee_dev.mac[6],ym_zigbee_dev.mac[7]);
    MY_DEBUG("shortaddr is [ %04X ]\n",ym_zigbee_dev.shortadd);
    
    /*if control msg*/
    if(uart_2.rx_msg.msg_type == YM_DEVICE_CONTROL_MSG || 
       uart_2.rx_msg.msg_type == YM_DEVICE_MAC_SINGLE_MSG)
    {
 ...
 /* write to file */
    /* if only quer address  */
    if( (ym_zigbee_recv->CMD == CMD_NODE_INFO_LOOKUP) &&
       (ym_zigbee_recv->ClusterID == CID_NODE_ALLMAC_LOOKUP ||
        ym_zigbee_recv->ClusterID == CID_NODE_MAC_LOOPUP) )
    {
    ...
    /*other msg -- include statu msg*/
    else
     {
       if(!change_dev(&ym_zigbee_dev)) {
         MY_DEBUG("%s,%d: change statu faild !\n",__func__,__LINE__);
         goto EXIT;
       }
     }
...
int uart1_user_cmd_do(void)
{
  ...
  if(!uart1_device) {
    MY_DEBUG("%s,%d: Can't find uart1 !\n",__func__,__LINE__);
    goto EXIT;
  }
  char recv_buff[12] = {0};
  int read_size = rt_device_read(uart1_device, 0, recv_buff, UART_RECV_SIZE);
  
  if(read_size != UART_RECV_SIZE)
    goto EXIT;
  
  rt_mutex_take(&uart_2.mutex,RT_WAITING_FOREVER);
  int write_size = rt_device_write(uart_2.uart, 0, recv_buff, UART_SEND_SIZE);
  rt_mutex_release(&uart_2.mutex);  
  
  MY_DEBUG("recv msg from uart 1: \n");
  for(int i = 0; i < 12; i++)
    MY_DEBUG("%02X ", recv_buff[i]);
  MY_DEBUG("\n");
  return uart_ok;
EXIT:
  return uart_err;
