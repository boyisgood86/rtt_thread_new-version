...
/*socket init*/
static void init_socket(Send_Recv_CMDA_t *zigbee_dev, const ym_dev_ctl_t *ym_dev, int number)
{
 /*ctrl cmd*/
...
{
  /* switch on or off cmd */
  zigbee_dev->CMD = CMD_KEYRELAY_OPRT;
  if((ym_dev->dev+number)->devcice_progress == 0)
    zigbee_dev->ClusterID = CID_KEYRELAY_KEYOFF;
  else
    zigbee_dev->ClusterID = CID_KEYRELAY_KEYON;
  
  /*switch value*/
  zigbee_dev->SendNode.Info_mac.U_info.info.KeyRelay.CtrlVal = \
    (ym_dev->dev+number)->devcice_progress;
  /*switch type select*/
  if((ym_dev->dev+number)->switch_type == 0) {
     zigbee_dev->SendNode.Info_mac.U_info.info.KeyRelay.CtrlSelect = 1;
  }
  else {
    zigbee_dev->SendNode.Info_mac.U_info.info.KeyRelay.CtrlSelect = \
    (ym_dev->dev+number)->switch_type;
  }
...
//  (ym_dev->dev+number)->short_address = 00;
 ...
 switch(device_type) {
     ...
     case YM_LIGHT_COLOR:
       ...
       break;
     case YM_SOCKET:
       init_socket(&zigbee_dev, ym_dev, number);
       break;       
     case YM_CURTAINS:
